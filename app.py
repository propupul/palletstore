...
def loaderio():
    return send_from_directory(
        os.path.join(os.path.dirname(
                'loaderio-50773513826b6129054099d8691c796c')),
                'loaderio-50773513826b6129054099d8691c796c.txt')
...
def store():
    ...
    remaining_location = [
        remaining.location for remaining in empty_bay if not remaining.location.endswith('3L') and not remaining.location.endswith('3R') and not remaining.location.endswith('3LS') and
                          not remaining.location.endswith('3RS')
                          ]
    unracked_location = [
        'A8-2L', 'A8-2R', 'A9-2L', 'A9-2R', 'A13-2L', 'A13-2R', 'A8-3LS', 'A9-3RS', 'A13-3LS', 'A13-3RS'
        ]
